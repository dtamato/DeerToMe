...
EBTNodeResult::Type UHunterDistanceToDeerCheckTask::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (NewHunter)
	{
		...
		if (HunterController && HunterController->GetDistanceTo(HunterController->PlayerCharacter) <= 2500.0f && bShotPlayer == false)
		{	
			//play sound here
			bShotPlayer = true;
			HunterController->PlayerCharacter->SetCurrentDeerState(DeerState::DeerState_Shot);
